...
foreach my $sample (@samples) {
    ...
    my $adapterFile = "/PATH/TO/TruSeq2-PE_trimmomatic.fa";    ### This file is based on a list of generic sequences applicable to the type of data generated by Illumina. We provide the exact file we used in our NewtCap repository.
...
system("/PATH/TO/bbmap_repair.sh"); ### Adjust the path to the place where you have the correct version (! it may be needed to adjust it) of your bash script, bbmap_repair.sh, which will in turn run the BBmap script repair.sh. We provide the bbmap_repair.sh script in our NewtCap repository.
...
foreach my $sample (@samples) {
    ...
    system("bwa mem -M /PATH/TO/Tdob_reference_7139unique.fasta $R1 $R2 | samtools view -bS - > $bam");   ### We provide this FASTA file that was used as a reference for read alignment in our NewtCap repository.
    $bwaFM->finish; 
}
...
" --intervals /PATH/TO/sorted_ref_7139unique_targetnames.list --merge-contigs-into-num-partitions 1 --tmp-dir /PATH/TO/TEMPORARY_FILES_FOLDER");   ### Note that this is a list based on the reference FASTA file used, and we provided this exact list/file in our NewtCap repository.
